...
**출력**: `콘크리트_50km이내.csv` 처럼 `{종류}_{거리}km이내.csv` 로 저장
(도로거리 가까운 순 정렬, 컬럼: 업체명·주소·거리(km)·시간·직선거리(km))

> `tmap_filter.py` 는 GUI 없이 단독 실행도 가능합니다:
> ```bash
> python tmap_filter.py --origin "부산 강서구 평강로 271" --max-km 50
> python tmap_filter.py --origin-latlon 35.16,128.99 --max-km 30 --buffer 1.3
> ```
